# A3 Trade studies
# imported in A1 KevTestCode and turned it into a funciton that outputted objective parameters for the A3 trade studies
"""
...
def sizing_function(AR, S_ref, tc):
    """
    Runs the aircraft sizing loop with specified geometry inputs.
    ...
    -------
    W_fuel_total : float
        Total fuel burned [lbf]
    W0 : float
        Maximum takeoff weight [lbf]
    """

    # --- Create parameter object with updated inputs ---
    params = AircraftParams(
        AR=AR,
        S_ref=S_ref,
        tc=tc
    ...
    results = run_sizing(
        params,
        W_payload=params.W_payload_a2a,  # default payload
        W0_guess=45000.0,
        tol=1.0,
        verbose=False
    )
    ...
    # --- Extract outputs ---
    W_fuel_total = results['W_fuel']
    W0 = results['W0']

    return W_fuel_total, W0


# def carpet_plot_W0_WS(tc_vals=[0.06, 0.08, 0.10]):
...
#                 # --- Iteratively solve for consistent S_ref ---
#                 W0_guess = 45000.0

#                 for _ in range(5):  # small inner loop
#                     S_ref = W0_guess / WS

#                     fuel, W0 = sizing_function(AR, S_ref, tc)

#                     W0_guess = W0  # update

#                 W0_line.append(W0)

#             W0_line = np.array(W0_line)

#             # Plot
#             ax.plot(AR_vals, W0_line,
#                     linestyle=linestyles[k],
#                     color=colors[i],
#                     marker='o',
#                     label=f"W/S={WS:.0f}" if k == 0 else None)

#         # Label t/c groups
#         ax.text(AR_vals[-1] + 0.1, W0_line[-1],
#                 f"t/c={tc}", fontsize=9)

#     # Formatting
#     ax.set_xlabel("Aspect Ratio (AR)")
#     ax.set_ylabel("Takeoff Weight W0 [lbf]")
#     ax.set_title("Carpet Plot: W0 vs AR, W/S, and t/c")
#     ax.grid(True)

#     ax.legend(title="Wing Loading W/S [lb/ft²]", fontsize=8)

#     plt.tight_layout()
#     plt.show()

# carpet_plot_W0_WS([0.06, 0.08, 0.10])
...
def carpet_plot_W0_WS_Mach(M_vals=[0.7, 0.85, 1.0]):
    ...
    for k, M in enumerate(M_vals):
        ...
        for i, WS in enumerate(WS_vals):
            ...
            for AR in AR_vals:
                ...
                # --- Iteratively enforce consistent W/S ---
                W0_guess = 45000.0

                for _ in range(5):
                    S_ref = W0_guess / WS

                    # --- Create params with updated Mach ---
                    params = AircraftParams(
                        AR=AR,
                        S_ref=S_ref,
                        M_cruise=M
                    ...
                    results = run_sizing(
                        params,
                        W_payload=params.W_payload_a2a,
                        W0_guess=W0_guess,
                        tol=1.0,
                        verbose=False
                    )
    ...
    ax.set_xlabel("Aspect Ratio (AR)")
    ax.set_ylabel("Takeoff Weight W0 [lbf]")
    ax.set_title("Carpet Plot: W0 vs AR, W/S, and Mach")
    ax.grid(True)
...
carpet_plot_W0_WS_Mach([0.7, 0.85, 1.0])