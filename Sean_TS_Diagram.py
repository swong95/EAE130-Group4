...
def calculate_weight_fraction(L_D_max, R, E, c, V):
    ...
    # IF DOING A2A CHANGE RETURN TO Wf_W0_a2a, IF DOING STRIKE RETURN Wf_W0_strike
    return Wf_W0_strike
...
# CHANGE THIS TO W_payload_a2a IF DOING AIR TO AIR MISSION, CHANGE TO W_payload_strike IF DOING STRIKE MISSION
...
def outer_loop_thrust_for_one_constraint(
    ...
    W_crew, W_payload,
    coef_1_turn_constraint, coef_2_turn_constraint,
    tol_T_rel=1e-3,          
    max_iter_T=100,
    ...
    for S_wing in S_wing_grid:
        ...
        for k in range(max_iter_T):
            ...
            TW_req = coef_1_turn_constraint/WS + coef_2_turn_constraint*WS
...
# Final parameters for weight estimation
L_D_max = 9             # LOOK AT FIG 3.5 FROM RAYMER TO ESTIMATE L/D MAX BASED ON WETTED AREA ASPECT RATIO
                        ## Using F-15 as reference id using extrapolation try 10.5 FIG 2.4 FROM Martins
L_D = 0.866 * L_D_max   # 86.6% of L/D_max is typical estimate for jets at cruise (pg 43 of Raymer)
...
T_total_curve, W0_curve, n_iter_T, T_hist_allS, W0_final, wconv_final, it_w_final, W0_hist_final = outer_loop_thrust_for_one_constraint(
    ...
    coef_1_turn_constraint=turn_coeff1,
    coef_2_turn_constraint=turn_coeff2,
    tol_T_rel=1e-6,
    max_iter_T=200,
    relax=1
)
...
plt.plot(S_wing_grid, T_total_curve, label='Converged T for Turn Constraint', marker='o')
plt.legend(loc='best')
plt.grid()
plt.show()