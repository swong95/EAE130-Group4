...
def compute_outputs(AR, S_w):
    ...
    W_prop = (W_en + W_emounts + W_firewall + W_ensect +
              W_tailpipe + W_encool + W_oilcool +
              W_starter + W_fuelsystanks)

    W_sys = (W_flightcont + W_instr + W_hydraulics +
             W_elec + W_avionics + W_furn + W_acai)

    # -------------------------
    # PAYLOAD + USEFUL (UPDATED)
    # -------------------------
    W_payload = N_AIM120C*W_AIM120C + N_AIM9X*W_AIM9X

    # pylons + pilot (added from script 2 logic)
    W_useful = 200 + (N_AIM120C*42.96) + (N_AIM9X*22.32)

    # -------------------------
    # TOTAL WEIGHT (UPDATED)
    # -------------------------
    W_total = W_struct + W_prop + W_sys + W_payload + W_useful + W_fuel_a2a
